...
- Логировать этапы pipeline, версию артефакта, среду и итоговый статус шага.
- Не логировать секреты, токены, содержимое secret variables и чувствительные фрагменты конфигурации.
- Для deploy и rollback фиксировать инициатора, commit/tag и результат post-checks.
- Логи должны позволять отличить сбой сборки, сбой деплоя и деградацию после выката.
